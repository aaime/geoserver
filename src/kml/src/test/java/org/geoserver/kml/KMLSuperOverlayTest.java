...
public class KMLSuperOverlayTest extends WMSTestSupport {
    ...
    protected void onSetUp(SystemTestData testData) throws Exception {
        ...
        testData.addVectorLayer(
                DISPERSED_FEATURES, Collections.emptyMap(), "Dispersed.properties", getClass(), catalog);
    ...
    public void testWorldBoundsSuperOverlay() throws Exception {
        Document document = getAsDOM("wms/kml?layers="
    ...
    public void testCrossingSuperoverlay() throws Exception {
        ...
        assertEquals(
                -105.2243,
                Double.parseDouble(xpath.evaluate("//kml:Document/kml:Folder/kml:LookAt/kml:longitude", document)),
                1E-4);
        assertEquals(
                40.0081,
                Double.parseDouble(xpath.evaluate("//kml:Document/kml:Folder/kml:LookAt/kml:latitude", document)),
                1E-4);
    }
    ...
    public void testGWCIntegration() throws Exception {
        Document document = getAsDOM(
                "wms/kml?layers=" + getLayerId(MockData.USA_WORLDIMG) + "&mode=superoverlay&superoverlay_mode=cached");
        // print(document);
