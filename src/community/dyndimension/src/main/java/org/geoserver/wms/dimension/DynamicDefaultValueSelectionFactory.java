...
public class DynamicDefaultValueSelectionFactory extends AbstractDispatcherCallback
        implements DimensionDefaultValueSelectionStrategyFactory, ExtensionPriority, ApplicationContextAware {
    ...
    private DefaultValueConfigurations getConfigurations(ResourceInfo resource) {
        DefaultValueConfigurations configurations =
                resource.getMetadata().get(DefaultValueConfigurations.KEY, DefaultValueConfigurations.class);
    ...
    private Map<String, Object> getDefaultValues(
            ResourceInfo resource, GetMapRequest getMap, DefaultValueConfigurations configurations) throws IOException {
    ...
    private Map<String, Object> buildDynamicDefaults(
            ResourceInfo resource, GetMapRequest getMap, DefaultValueConfigurations configsBean) throws IOException {
                ...
                if (!foundExplicitDefault) {
                    if (!hasDynamicConfiguration(configurations, dimensionName)) {
                        ...
                        if (strategy == null) {
                            ...
                            Object staticDefault = strategy.getDefaultValue(
                                    resource, dimensionName, di, getDimensionClass(dimensionName));
    ...
    private boolean hasDynamicConfiguration(List<DefaultValueConfiguration> configurations, String dimensionName) {
        for (DefaultValueConfiguration config : configurations) {
    ...
    private Object getDynamicDefault(
            ResourceInfo resource, Map<String, Object> result, DefaultValueConfiguration config, String dimensionName)
            throws IOException {
        // get the dimension info
        DimensionInfo di =
                resource.getMetadata().get(getDimensionMetadataKey(config.getDimension()), DimensionInfo.class);
        ...
        if (config.policy == DefaultValuePolicy.STANDARD) {
            if (di == null) {
                ...
                if (LOGGER.isLoggable(Level.FINE)) {
                    LOGGER.fine("Computed default value for dimension "
                            + dimensionName
            ...
            if (resource instanceof FeatureTypeInfo) {
                ...
                DimensionDefaultValueSelectionStrategy delegateStrategy =
                        delegate.getStrategy(resource, getDimensionMetadataKey(config.getDimension()), di);
                Object value = getRasterDefaultValue((CoverageInfo) resource, config, result, delegateStrategy);
                return value;
            } else {
                throw new IllegalArgumentException(
                        "Don't know how to handle domain restriction for layers of type " + resource.getClass());
            }
        }
    ...
    private Object applyExpression(
        ...
        try {
            ...
            throw new ServiceException(
                    "Failed to parse value returned from dynamic default value expression: " + strValue, e);
        }
    }
    ...
    private Object getRasterDefaultValue(
        ...
        if (delegateStrategy instanceof FixedValueStrategyImpl) {
            Class<?> dimensionClass = getDimensionClass(config.getDimension());
            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.fine("Dynamic domain restriction on dimension "
                        + dimensionName
                        + " in layer "
                        + resource.prefixedName()
        ...
        for (Map.Entry<String, Object> entry : restrictions.entrySet()) {
            ...
            if (descriptor == null) {
                throw new ServiceException(
                        "Could not find dimension " + name + " in coverage reader backing " + resource.prefixedName());
            }
        ...
        if (dd == null) {
            throw new ServiceException("Could not find dimension "
                    + dimensionName
        ...
        if (LOGGER.isLoggable(Level.FINE)) {
            LOGGER.fine("Computed default value for "
                    + dimensionName
                    + " in layer "
                    + resource.prefixedName()
                    + " using "
    ...
    private Object getVectorDefaultValue(
            FeatureTypeInfo resource, DefaultValueConfiguration config, Map<String, Object> restrictions)
            throws IOException {
        DimensionInfo di =
                resource.getMetadata().get(getDimensionMetadataKey(config.getDimension()), DimensionInfo.class);
        ...
        if (LOGGER.isLoggable(Level.FINE)) {
            LOGGER.fine("Computing default value for"
                    + config.getDimension()
                    + " in layer "
                    + resource.prefixedName()
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        ...
        delegate = applicationContext.getBean(DimensionDefaultValueSelectionStrategyFactoryImpl.class);
