    ...
    public TaskResult run(TaskContext ctx) throws TaskException {
        ...
        if (createLayer) {
            ...
            createStyle =
                    !restManager
                            .getReader()
                            .existsStyle(wsStyle, layer.getDefaultStyle().getName());
            createStore =
            ...
            createResource =
                    !(storeType == StoreType.DATASTORES
                            ? restManager
                                    .getReader()
                                    .existsFeatureType(ws, store.getName(), resource.getName())
                            : restManager
                                    .getReader()
                                    .existsCoverage(ws, store.getName(), resource.getName()));

            try {

                if (!createResource) {
                    // we are in the awkward situation where the resource exists, but not the layer
                    // the only solution is to delete the rogue resource, otherwise we cannot create
                    // the layer
                    if (!restManager
                            .getPublisher()
                            .removeResource(ws, storeType, store.getName(), resource.getName())) {
                        throw new TaskException(
                                "Failed to delete resource " + ws + ":" + resource.getName());
                    }
                }
                ...
                if (createWorkspace) { // workspace doesn't exist yet, publish
                    ...
                    try {
                        if (!restManager
                                .getPublisher()
                                .createWorkspace(
                                        ws, new URI(catalog.getNamespaceByPrefix(ws).getURI()))) {
                ...
                for (MetadataLinkInfo mdli : resource.getMetadataLinks()) {
                    re.addMetadataLinkInfo(
                            mdli.getType(), mdli.getMetadataType(), mdli.getContent());
                }
                ...
                if ((storeType == StoreType.DATASTORES
                    ...
                    if (!restManager
                            .getPublisher()
                            .configureResource(ws, storeType, store.getName(), re)) {
                        throw new TaskException(
                    ...
                    if (!restManager
                            .getPublisher()
                            .createResource(ws, storeType, store.getName(), re)) {
                        throw new TaskException(
                ...
                if (createStyle) { // style doesn't exist yet, publish
                    ...
                    if (!restManager
                            ...
                            .publishStyleInWorkspace(
                                    ...
                                    layer.getDefaultStyle().getName())) {
                        throw new TaskException(
                                "Failed to create style " + layer.getDefaultStyle().getName());
                    }
                }
                ...
                if (createStore) {
                    restManager
                            .getPublisher()
                            .removeStore(ws, store.getName(), storeType, true, Purge.ALL);
                }
                if (createStyle) {
                ...
                if (createLayer) {
                    if (createResource) {
                        if (!(storeType == StoreType.COVERAGESTORES
                                ? restManager
                                        .getPublisher()
                                        .unpublishCoverage(ws, store.getName(), resource.getName())
                                : restManager
                                        .getPublisher()
                                        .unpublishFeatureType(
                                                ws, store.getName(), resource.getName()))) {
                            throw new TaskException(
                                    "Failed to remove layer/resource "
                                            + ws
                                            + ":"
                                            + resource.getName());
                        }
    ...
    public void cleanup(TaskContext ctx) throws TaskException {
        ...
        if (restManager.getReader().existsLayer(ws, layer.getName(), true)) {
            if (!(storeType == StoreType.COVERAGESTORES
                    ...
                    : restManager
                            .getPublisher()
                            .unpublishFeatureType(ws, store.getName(), resource.getName()))) {
                throw new TaskException(
                        "Failed to remove layer/resource " + ws + ":" + resource.getName());
            }
            if (!restManager
                    .getPublisher()
                    .removeStore(ws, store.getName(), storeType, false, Purge.ALL)) {
                if (mustCleanUpStore()) {
                    throw new TaskException("Failed to remove store " + ws + ":" + store.getName());
                } else {
                    LOGGER.log(
                            Level.INFO,
                            "Failed to clean-up datastore "
                                    + store.getName()
