...
public class GHRSSTExtensionPanel extends NetCDFExtensionPanel {
     ...
     * (TODO)
     */
    static final List<String> PRODUCT_STRINGS = Arrays.asList(
            "AVHRR7_D",
            "AVHRR9_D",
            "AVHRR10_D",
            "AVHRR11_D",
    ...
    public GHRSSTExtensionPanel(String id, IModel<?> model, NetCDFPanel parent) {
        ...
        enabled = new CheckBox(
                "enabled", new MetadataMapModel<>(metadataModel, GHRSSTEncoder.SETTINGS_KEY, Boolean.class));
        add(enabled);

        WebMarkupContainer settings = new WebMarkupContainer("settings");
        settings.setOutputMarkupId(true);
        add(settings);
        this.rdac = getAutocompleter(
                "rdac", new MetadataMapModel<>(metadataModel, GHRSSTEncoder.SETTINGS_RDAC_KEY, String.class), RDACS);
        rdac.setRequired(true);
        settings.add(rdac);
        this.processingLevel = getAutocompleter(
                "processingLevel",
        ...
        this.sstType = getAutocompleter(
                "sstType",
                new MetadataMapModel<>(metadataModel, GHRSSTEncoder.SETTINGS_SST_TYPE, String.class),
                SST_TYPES);
        sstType.setRequired(true);
        settings.add(sstType);
        this.productString = getAutocompleter(
                "productString",
        ...
        return new AutoCompleteTextField<String>(id, model, settings) {
            ...
            protected Iterator<String> getChoices(String input) {
                ...
                return choices.stream()
                        .filter(c -> c.toLowerCase().contains(reference))
                        .iterator();
            }
        };
    }
    ...
    public void convertInput(NetCDFSettingsContainer settings) {
        ...
        settings.getMetadata().put(GHRSSTEncoder.SETTINGS_PROCESSING_LEVEL_KEY, processingLevel.getModelObject());
        settings.getMetadata().put(GHRSSTEncoder.SETTINGS_SST_TYPE, sstType.getModelObject());
        settings.getMetadata().put(GHRSSTEncoder.SETTINGS_PRODUCT_STRING, productString.getModelObject());
    }
}
