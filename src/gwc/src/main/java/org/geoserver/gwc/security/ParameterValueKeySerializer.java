 ...
 *
 * <p>Register custom implementations as Spring beans. {@link AccessLimitsKeyBuilder} collects all such beans at startup
 * and dispatches by value type via {@link Class#isAssignableFrom}. Duplicate registrations for the same value type fail
 * at startup with a descriptive error.
 *
 * <p>{@link #toKey} must be deterministic: identical inputs must produce identical output, and inputs that result in
 * different tile content must produce different output.
 *
...
public interface ParameterValueKeySerializer<T> {
    ...
    /** Value class this serializer handles. Matched via {@link Class#isAssignableFrom}. */
