...
/**
 * Normalizes filters to reduce cache key fragmentation without changing semantics. Extends
 * {@link SimplifyingFilterVisitor} (which handles constant folding, double-negation, AND/OR flattening) and adds:
 *
 * <ul>
 *   <li>Comparison swap: {@code Literal op PropertyName} → {@code PropertyName inverted-op Literal}
 *   <li>AND/OR operand sort: stable ordering by (class name, string representation) to absorb producer-side ordering
 *       variation
...
class NormalizingFilterVisitor extends SimplifyingFilterVisitor {
    ...
    /** Stable total order for filter children: class name first, then string representation. */
    ...
    private static List<Filter> sorted(List<Filter> children) {
        return children.stream().sorted(FILTER_ORDER).toList();
    }
}
