...
public class GeoJSONFeatureWriter {
    ...
    public GeoJSONFeatureWriter(GeoServer gs) {
        this.gs = gs;
    }

    /**
     * Writes a list of {@link FeatureCollection} on the specified output stream
     * 
     * @param featureCollections
     * @param output
     * @param jsonp - if true a JSONP output will be encoded
     * @param featureBounding - if true the features bounds will be written to output
     * @throws IOException
     */
    public void write(List featureCollections, OutputStream output) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < featureCollections.size(); i++) {
                FeatureCollection collection = (FeatureCollection) featureCollections.get(i);
